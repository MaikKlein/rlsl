...
fn create_graphics_pipeline<B: Backend, D: Device<B>>(
    device: &D,
    ...
    let pipeline = {
        let spv_module = device
            .create_shader_module(include_bytes!("../../shader.spv"))
            .unwrap();
        ...
        */
        ...
        device.destroy_shader_module(spv_module);
        /*
        #[cfg(all(feature = "metal", feature = "metal_argument_buffer"))]
    ...
    let (frame_images, framebuffers) = match backbuffer {
        Backbuffer::Images(images) => {
            ...
            let pairs = images
                ...
                .map(|image| {
                    let rtv = device
                        .create_image_view(&image, surface_format, Swizzle::NO, COLOR_RANGE.clone())
                        .unwrap();
                    (image, rtv)
                })
                .collect::<Vec<_>>();
